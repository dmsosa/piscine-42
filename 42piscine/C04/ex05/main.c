...
int		ft_atoi_base(char *str, char *base)
{
	...
	while (str[i] >= 48 && str[i] <= 57)	
	{
		result += base_len * result + find_index(str[i], base);
	}
	return (result * sign);
}
...
int ft_atoi_base(char *str, char *base)
{
	//DIVIDE AND CONQUER was ich macht? ich gehe raus mit meine Tante um seine Sache zu bringen, und dann essen wir in der Srasse, dann komme ich zuruck, ein bisschen mehr arbeit und Yoga und Ephesius lernen, und dann kann ich widerschlafen, oh man!!!!!!
    //atoi base
